...
import Tendril from './classes/Trendil';
import HueOscillator from './classes/HueOscillator';
...
function DancingLines(props) {
  ...
  const loop = () => {
    ...
    if (!runningRef.current || !canvasRef.current || tendrilsRef.current.length === 0) return;
    ...
    for (let i = 0, tendril; i < tendrils.length; i++) {
      tendril = tendrils[i];
      tendril.update();
      tendril.draw();
    }
